...
pdf("wgs_wts_lineplot.pdf",width=15,height=15)
...
pal<-iwanthue(93)
a3<-ggplot(v[grepl("Ammonia|Complete|Nitrite oxidation|Nitrite reduction|DNRA|Nitrate reduction|Nitric oxide|Nitrous oxide|Nitrite reduction",v$Specific.pathway), ],col=pal, aes(sample, values, fill = taxac)) +
    theme_bw()  +
    geom_bar(position="stack", stat="identity") +
...
pal<-iwanthue(150)
a4<-ggplot(wgs_wts[grep("Cyanophycin|N fixation|Phosphatase|Phosphate Inorganic Transporter|Phosphate Regulon|PhoU|Specific Transport",wgs_wts$Specific.pathway), ],col=pal, 
    aes(sample, values, fill = taxac)) +
    theme_bw()  +
    geom_bar(position="stack", stat="identity") +
    facet_grid( Specific.pathway~place , scales = "free_y",labeller = label_wrap_gen(width=15))+
    scale_fill_manual(values = pal)+scale_x_discrete(name="Distance (km)", labels=scaleFUN(as.numeric(as.character(map$Distance[1:9]))))+
    theme(legend.position="bottom",axis.text.x=element_text(angle=90, color ="black",hjust=1,vjust=0.5,size =15),axis.title=element_text(size=18, color ="black"),
    legend.text=element_text(size=16, color ="black"),axis.text.y=element_text(size=15, color ="black"),plot.title = element_text(face = 'bold', size = 20,hjust=0.5),
    strip.text.x = element_text(size = 16, color ="black" ),strip.text.y = element_text(size = 16, color ="black" ),
    panel.grid.major = element_blank(),panel.grid.minor = element_blank())+
    guides(fill=guide_legend(ncol=3))+ labs(title = "Phosphorus", x = "Sites", y = "RPKM")
p4=set_panel_size(a4,width = unit(5.5, "cm"),height = unit(5.5, "cm"))

pal<-iwanthue(88)
a5<-ggplot(wgs_wts[grep("Sulfate reduction|Sulfite reduction|Sulfide oxidation|Sulfur oxidation|Thiosulfate",wgs_wts$Specific.pathway), ],col=pal, aes(sample, values, fill = taxac)) +
    theme_bw()  +
    geom_bar(position="stack", stat="identity") +
    facet_grid( Specific.pathway~place , scales = "free_y",labeller = label_wrap_gen(width=15))+
    scale_fill_manual(values = pal)+scale_x_discrete(name="Distance (km)", labels=scaleFUN(as.numeric(as.character(map$Distance[1:9]))))+
    theme(legend.position="bottom",axis.text.x=element_text(angle=90,hjust=1, color ="black",vjust=0.5,size =15),axis.title=element_text(size=18, color ="black"),
          legend.text=element_text(size=16, color ="black"),axis.text.y=element_text(size=15, color ="black"),plot.title = element_text(face = 'bold', size = 20,hjust=0.5),
          strip.text.x = element_text(size = 16, color ="black" ),strip.text.y = element_text(size = 16, color ="black" ),
          panel.grid.major = element_blank(),panel.grid.minor = element_blank())+
    guides(fill=guide_legend(ncol=3))+ labs(title = "Sulfate and sulfite reduction", x = "Sites", y = "RPKM")
p5=set_panel_size(a5,width = unit(5.5, "cm"),height = unit(5.5, "cm"))

v=wgs_wts[grep("Sulfur",wgs_wts$Function), ]
pal<-iwanthue(88)
a6<-ggplot(v[grep("dsr",v$Gene), ],col=pal, aes(sample, values, fill = taxac)) +
  theme_bw()  +
  geom_bar(position="stack", stat="identity") +
...
pdf("taxa_barplot.pdf",width=25,height=58)
marrangeGrob(grobs=list(p2,p1,p3,p4,p5,p6),  nrow = 1,ncol = 1)
dev.off()
...
PHOSPHATE=taxa_wts_all[grep("Phosphate Specific Transport System|Phosphate Inorganic Transporter|Phosphate Regulon",taxa_wts_all$X3), ] %>%select(f1:S7R3) %>% colSums()
